...
public class WordDuoList {
    ...
    public WordDuoList(String[] words){
        /* part A */
        allDuos= new ArrayList<WordDuo>(words.length);
		
        for(int x = 0;x<words.length;x++){
            for(int y=x+1;y<words.length;y++){
                allDuos.add(new WordDuo(words[x],words[y]));
            }
        }
    ...
    public void moveMatchesToTop(){
        // /* part c */
        //i feel like there's a simpler way to do this but i kept getting "concurrent modification exceptions", so this works fine
        ArrayList<WordDuo> temp = new ArrayList<WordDuo>(1);
        for(WordDuo word:allDuos){
            if(word.getFirst().equals(word.getSecond())){
                temp.add(word);
            }
        }
        for(WordDuo word:temp){
            if (allDuos.contains(word)){
                allDuos.remove(word);
            }
        }
